...
class IndividualAIService:
    def __init__(self, socketio_instance):
        ...
        print("✅ Individual AI Service initialized (using core AI agent)!")
    ...
    def handle_individual_message_sync(self, user_id: str, room_id: str, message_content: str) -> Optional[str]:
        ...
        try:
            print(f"🤖 Individual AI Service: Starting to handle message from user {user_id} in room {room_id}")
            print(f"🤖 Message content: {message_content[:100]}...")
            
            ai_agent = get_ai_agent()
            if not ai_agent or not ai_agent.client:
                print("🚫 Core AI agent not available")
                return None

            # Generate personal room ID
            personal_room_id = self.get_personal_room_id(room_id, user_id)
            print(f"🤖 Processing individual message for personal room: {personal_room_id}")
            ...
            print(f"🤖 Adding user message to AI agent context: {message_data}")
            
            # Add the user message to the core AI's conversation context
            ai_agent.add_message_to_context(message_data)

            print(f"🤖 Calling centralized AI decision for personal room: {personal_room_id}")
            ...
            print(f"🤖 AI decision result - should_respond: {should_respond}, response length: {len(ai_response) if ai_response else 0}")
            
            if should_respond and ai_response:
                print(f"🤖 AI will respond with: {ai_response[:100]}...")
                ...
                print("🤖 AI decided not to respond, providing fallback response")
            ...
            print(f"❌ Error handling individual message for user {user_id}: {e}")
            import traceback
            traceback.print_exc()
            return None
    ...
    def copy_room_context_to_personal(self, original_room_id: str, user_id: str):
        ...
        try:
            ...
            if not original_context:
                print(f"ℹ️ No original context found for room {original_room_id}")
                return
            ...
            print(f"✅ Copied context from {original_room_id} to personal room {personal_room_id}")
            
        except Exception as e:
            print(f"❌ Error copying context to personal room: {e}")
    ...
    def clear_individual_conversation(self, user_id: str, room_id: str):
        ...
        try:
            ...
            if personal_room_id in ai_agent.conversation_history:
                ...
                print(f"🧹 Cleared individual conversation for user {user_id} in room {room_id}")
        except Exception as e:
            print(f"❌ Error clearing individual conversation: {e}")
    ...
    def clear_room_conversations(self, room_id: str):
        ...
        try:
            ...
            print(f"🧹 Cleared individual AI conversations for {users_cleared} users in room {room_id}")
        except Exception as e:
            print(f"❌ Error clearing room conversations: {e}")
